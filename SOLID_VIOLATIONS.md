...
## src/main/java/ru/mipt/bit/platformer/game/GameSession.java
...
## src/main/java/ru/mipt/bit/platformer/input/InputHandler.java
...
## src/main/java/ru/mipt/bit/platformer/view/LevelGraphics.java
...
## src/main/java/ru/mipt/bit/platformer/view/GameField.java
...
- `src/main/java/ru/mipt/bit/platformer/level/TmxLevelLoader.java`: читает размеры карты из TMX и переносит в `LevelData`.
 - `src/main/java/ru/mipt/bit/platformer/input/InputHandler.java`: выделен источник ввода `InputSource`, обработчик больше не зависит напрямую от `Gdx.input`; команды ввода оформлены через публичный интерфейс `InputCommand`, добавлена возможность инъекции/расширения набора команд (OCP). Сохранен совместимый конструктор, создающий дефолтные биндинги. Удалены конструкторы, создающие `MovementRules` внутри (DIP/SRP).
 - `src/main/java/ru/mipt/bit/platformer/input/InputSource.java`: новая абстракция источника ввода (DIP).
 - `src/main/java/ru/mipt/bit/platformer/input/GdxInputSource.java`: адаптер к `Gdx.input`.
 - `src/main/java/ru/mipt/bit/platformer/game/GameWorldBuilder.java`: вынесена сборка мира/спавн/правила движения/ввод/AI из `GameSession` (SRP, DIP).
 - `src/main/java/ru/mipt/bit/platformer/game/GameSession.java`: стал координатором кадра — подписка на рендер, снимок занятости, сбор команд ввода/AI в очередь, исполнение и тик мира (SRP, OCP для командного пайплайна).
 - `src/main/java/ru/mipt/bit/platformer/AIHandler.java`: AI теперь добавляет команды в очередь вместо прямых вызовов, случайность инжектируется (DIP, OCP для пайплайна).
 - `src/main/java/ru/mipt/bit/platformer/view/GameField.java` и `LevelGraphics.java`: выделены в пакет view для развязки мира и рендера (DIP/арх. границы).

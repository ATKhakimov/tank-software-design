...
## Хронология домашек
### 2025-11-24 (Spring IoC)
- Подключён Spring (spring-context) для Inversion of Control.
- `GameDesktopLauncher` превращён в компонент, получающий зависимости через конструктор (стратегия ботов, контроллер полосок жизней, фабрика мира).
- Конфигурация `GameConfig` объявляет бины: стратегия (`BotStrategy` по свойству `ai`), контроллер жизней, `WorldModelFactory` (bullet.speed, bullet.damage).
- Точка входа создаёт Spring контекст и извлекает `GameDesktopLauncher` вместо ручного new.
- Параметры пуль и AI теперь могут задаваться системными свойствами (`-Dbullet.speed`, `-Dbullet.damage`, `-Dai`).
### 2025-11-17
- Добавлены стрельба и урон: команда стрельбы у игрока (Space) и у ботов (вероятность). Используется `Shooter` для инверсии зависимостей.
- Введён логический мир `WorldModel` (наблюдаемый объект) и паттерн Наблюдатель (`WorldObserver`) для уведомления графического уровня об добавлении/удалении объектов (танк, препятствие, пуля).
- Пули: создаются перед танком по его ориентации, перемещаются дискретно (tick), при столкновении (танк/дерево/край/другая пуля) исчезают; при попадании наносят урон и могут убить танк (удаление из мира).
- Здоровье танков (0..1) визуализируется через декоратор (вкл/выкл клавишей L) — объединение Command + Decorator.
- Цикл кадра разделён на 4 шага: сбор команд (ввод/AI), выполнение команд, tick мира (обновление пуль/смерти), отрисовка (синхронно с уведомлениями наблюдателя).
- AI: случайный выбор движения или стрельбы; стратегии движения остаются переключаемыми.
### 2025-10-29
